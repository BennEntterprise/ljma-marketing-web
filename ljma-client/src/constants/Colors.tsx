// Pinks/Purples
export const flower: string = '#fec5e9'
export const magenta: string = '#fa4faa'
export const darkMagenta: string = '#FF00B3'
export const lightPurple: string = '#c493f9'
export const purple: string = '#a800d6'
//Blues
export const tortise: string = '#99d5ca'
export const teal: string = '#00c0c3'
export const seagreen: string = '#01a589'

//White/Black/Grey
export const charcoal: string = '#202020'
export const obsidian: string = '#090909'
export const white: string = '#fff'
export const black: string = '#000'

export const offBlack: string = '#010101'
